  ...
  public static search(query: string, typeFilter?: string): SearchResultItem[] {
    ...
    if (!typeFilter || typeFilter === 'person') {
      ...
      for (const p of matchedPeople.slice(0, 10)) {
        const pPos = s.person_positions.find((pp) => pp.person_id === p.id && pp.status === 'published');
        const pos = pPos ? s.positions.find((pos) => pos.id === pPos.position_id) : null;
        const inst = pos?.institution_id ? s.institutions.find((i) => i.id === pos.institution_id) : null;

        results.push({
          id: p.id,
          type: 'person',
          title: p.full_name,
          subtitle: pos ? `${pos.name} (${inst?.name || ''})` : 'Персона / Депутат',
          url: `/people/${p.slug}`,
          badge: 'Депутат / Руководитель',
        });
      }
    }
    ...
    if (!typeFilter || typeFilter === 'competence') {
      ...
      for (const c of matchedCompetences.slice(0, 10)) {
        const inst = c.institution_id ? s.institutions.find((i) => i.id === c.institution_id) : null;
        results.push({
          id: c.id,
          type: 'competence',
          title: c.name,
